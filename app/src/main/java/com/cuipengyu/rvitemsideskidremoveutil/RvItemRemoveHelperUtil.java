 ...
 * Instructions ：
 */
public class RvItemRemoveHelperUtil extends ViewGroup {
    //存储的是当前正在展开的View
    private static RvItemRemoveHelperUtil mViewCache;
    ...
    public RvItemRemoveHelperUtil(Context context) {
        this(context, null);
    }

    public RvItemRemoveHelperUtil(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RvItemRemoveHelperUtil(Context context, AttributeSet attrs, int defStyleAttr) {
...
/*                    if (!mScroller.isFinished()) {
                    ...
                    scrollBy((int) (gap), 0);//滑动使用scrollBy ,让其手指滑动
                    //越界修正
                    if (isLeftSwipe) {//左滑
                        if (getScrollX() < 0) {
                            scrollTo(0, 0);
